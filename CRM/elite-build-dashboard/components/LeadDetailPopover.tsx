...
export function LeadDetailPopover({ lead }: LeadDetailPopoverProps) {
  ...
  return (
    <div className="w-80 bg-mn-card border border-mn-border rounded-2xl shadow-2xl shadow-black/40 p-5 space-y-4">
      {/* Header */}
      <div className="flex items-start justify-between">
        <div>
          <h3 className="font-black text-mn-h1 text-base">{raw.lead_name}</h3>
          <p className="text-xs text-mn-text-muted mt-0.5">{lead.source}</p>
        </div>
        {lead.ai_audit && (
      ...
      {/* Details Grid */}
      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-mn-border/30">
        <div>
      ...
      {(lead.utm?.campaign || (lead.interested_properties && lead.interested_properties.length > 0)) && (
        <div className="pt-3 border-t border-mn-border/30 space-y-2">
      ...
      {/* Match */}
      {lead.suggested_plot && (
        <div className="pt-3 border-t border-mn-border/30 flex items-center gap-2">
